...
| struct [Md5Key](./WiseOwl.Casc/Md5Key.md) | A 128-bit CASC key (the MD5-derived hash CASC uses to address content). Stored as two UInt64 halves so the type is a small, allocation-free value with O(1) equality on every target framework (including `netstandard2.0`, which lacks inline arrays). |
