...
| [WindowPieces](TiledStyleDefinition/WindowPieces.md) { get; set; } | FR-C14 R10 / FR-C19 — for the `TiledWindowPieces` variant, the 9 piece handles of the 9-slice, at blob `+0x60..+0x80` in row-major 3×3 order: `[TL, T, TR, L, C, R, BL, B, BR]` (index 4 = the centre fill). Each is a texture frame handle (resolve via the icon-frame index); compose the 9-slice by drawing the corners at native size (scaled by [`ImageScale`](./TiledStyleDefinition/ImageScale.md)) in the cell corners, the edges stretched between them, and the centre filling the interior. Empty for non-window-pieces variants (use [`SourceImageHandle`](./TiledStyleDefinition/SourceImageHandle.md) + the NSlice fields). |
